...
\begin{itemize}
  ...
  \item \textbf{Contextual integrity of SEV proofs.} Proofs presented to one relying party must not be usable by, or correlatable with proofs presented to, other relying parties. This prevents aggregation of SEV information across services.
\end{itemize}
...
\begin{description}
  ...
  \item[G5: Contextual binding (anti-aggregation).] SEV proofs must be cryptographically bound to the requesting relying party such that: (i) a proof generated for $\textsf{RP}_A$ cannot be verified by $\textsf{RP}_B$; (ii) colluding relying parties cannot combine proofs to reconstruct a richer profile than any single RP could obtain; and (iii) global ``social credit'' scoring via cross-service SEV aggregation is technically infeasible.
    \item[G6: Passwordless authentication with recovery.] A user should be able to authenticate without passwords, using only control of the DID and selected SEL credentials, and regain access in the event of key loss via a protocol that leverages SEL-derived recovery factors without introducing centralized custodians.
\end{description}
...
\[
  I = (\mathsf{did}, \mathsf{sk}, \mathsf{pk}, \mathsf{DD}),
\]
where $\mathsf{did} \in \mathsf{DID}$, $(\mathsf{sk}, \mathsf{pk})$ is a signing keypair, and $\mathsf{DD}$ is the corresponding DID document.

In a SEL-DID system, we extend $I$ to
\[
  I_{\text{SEL}} = (\mathsf{did}, \mathsf{sk}, \mathsf{pk}, \mathsf{DD}, \mathcal{C}, \mathcal{E}),
\]
where:
\begin{itemize}
...
\[
  \mathbf{X} = \bigparallel_{p \in \mathcal{P}} \mathbf{x}_p,
\]
...
\paragraph{Terminology clarification.} The term ``entropy'' in SEV refers to \emph{behavioral unpredictability}---the statistical complexity that organic human activity accumulates over time---rather than a single information-theoretic entropy value. The SEV is a \emph{feature vector}, not a scalar. However, several components of $\mathbf{x}_p$ are themselves computed using Shannon entropy. For a discrete behavioral distribution $P = (p_1, \ldots, p_m)$ (e.g., posting times bucketed by hour, or content categories), we define
\[
  H(P) = -\sum_{i=1}^{m} p_i \log_2 p_i,
\]
where $0 \log 0 \triangleq 0$. Features such as ``posting regularity'' and ``diversity entropy'' in Table~\ref{tab:features} are instantiations of $H(P)$ over appropriate behavioral distributions. This formulation ensures that accounts with highly predictable (low-entropy) or highly erratic (potentially scripted) behavior score differently from accounts exhibiting the moderate, organic variability typical of genuine users.
...
\subsubsection{Social Continuity Decay}

To counter the ``Market for Lemons'' problem---whereby adversaries purchase aged but dormant accounts to inherit their apparent longevity---we require $C(t)$ to incorporate an explicit decay term that penalizes extended periods of inactivity. Concretely, let $t_{\text{last}}$ denote the timestamp of the most recent verifiable behavioral event (e.g., fresh credential issuance, active platform interaction). We define:
\[
  C(t) = h\big(\mathcal{E}(t)\big) \cdot D(t - t_{\text{last}}),
\]
where $D(\delta)$ is a monotonically decreasing decay function with $D(0) = 1$ and $\lim_{\delta \to \infty} D(\delta) = 0$. A natural choice is exponential decay:
\[
  D(\delta) = e^{-\lambda \delta},
\]
where $\lambda > 0$ is a decay rate parameter (e.g., $\lambda = 0.1$ per month). Under this formulation, an account that was created years ago but has been ``parked'' without activity will see its continuity score decay toward zero, regardless of its nominal age. In contrast, a genuinely active human who maintains consistent behavioral traces across platforms will continuously refresh $t_{\text{last}}$, keeping $D(\cdot)$ close to unity.

This decay mechanism directly addresses the economic incentives underlying account markets: purchasing a dormant account with a long history provides no advantage if the buyer cannot also reproduce the seller's ongoing behavioral patterns. The cost of maintaining plausible activity across multiple platforms over extended periods---responding to messages, generating organic content, sustaining social ties---remains fundamentally difficult to scale, making ``parked'' bot accounts substantially less valuable than accounts backed by active human behavior.
...
\node[box, above=of vault] (did) {DID + Keys\\$(\mathsf{sk}, \mathsf{pk})$};
...
\subsubsection{User-Generated Proofs via ZK-TLS}
\label{sec:zk_tls}

The credential issuance model described above assumes that Web2 platforms are willing to act as SEV issuers---computing feature vectors and signing verifiable credentials on behalf of users. In practice, this assumption faces a significant obstacle: the \emph{Platform Issuer Bottleneck}. Most large platforms have little incentive to issue credentials that strengthen identities outside their ecosystems, and many actively restrict API access to protect their data assets.

ZK-TLS protocols, such as TLSNotary and DECO, offer a promising alternative that removes the dependency on platform cooperation entirely.\cite{tlsnotary,deco} These protocols allow a user to generate cryptographic proofs about data received during a standard TLS session with any web server, without requiring the server to be aware of or participate in the proof generation.

\paragraph{How ZK-TLS works.} In a typical ZK-TLS flow:

\begin{enumerate}
  \item The user establishes a standard TLS connection to a Web2 platform (e.g., their social network profile page, account settings, or activity dashboard).
  \item A notary or multi-party computation (MPC) protocol participates in the TLS handshake in a way that allows the session transcript to be selectively revealed later, while preserving the authenticity guarantees of TLS.
  \item After the session, the user can generate a proof that specific data appeared in the authenticated server response---for example, that their account was created before a certain date, that they have more than $N$ followers, or that their username matches a particular pattern.
  \item Crucially, the user can redact sensitive portions of the response before generating the proof, revealing only the minimal information needed for the claim.
\end{enumerate}

\paragraph{Integrating ZK-TLS into SEL-DIDs.} ZK-TLS enables a new class of SEV credential that we call \emph{user-generated platform attestations}. Instead of waiting for a platform to issue a VC, the user's SEL agent can:

\begin{enumerate}
  \item Navigate to relevant pages on Web2 platforms (e.g., account creation date, follower counts, activity summaries) using a ZK-TLS-enabled client.
  \item Capture authenticated responses and generate selective disclosure proofs for the specific features needed by the SEV schema $\mathsf{Schema}_p$.
  \item Package the proof into a verifiable credential of the form:
  \[
    c_{p}^{\text{ZK-TLS}} = \big( \mathsf{id}_p, \mathsf{did}, \mathbf{y}_p, \pi_{\text{TLS}}, \textsf{meta}_p \big),
  \]
  where $\pi_{\text{TLS}}$ is the ZK-TLS proof attesting that $\mathbf{y}_p$ was derived from an authentic TLS session with platform $p$, and the credential is self-signed by the user's DID (since the authenticity derives from $\pi_{\text{TLS}}$ rather than from a platform signature).
\end{enumerate}

\paragraph{Benefits of user-generated proofs.}

\begin{itemize}
  \item \textbf{Platform-agnostic.} ZK-TLS works with any platform that serves data over HTTPS, without requiring API access or platform cooperation. This directly addresses design goal G3 (platform-agnostic continuity).
  \item \textbf{User-controlled timing.} Users can generate proofs at any time, not just when platforms choose to issue credentials. This improves responsiveness and reduces dependence on platform availability.
  \item \textbf{Selective disclosure by construction.} Because the user controls which portions of the TLS response are revealed in the proof, privacy is built into the issuance process rather than added as a layer on top.
  \item \textbf{Resistance to platform censorship.} A platform cannot selectively refuse to issue credentials to targeted users, since the user generates the proof unilaterally from public-facing data.
\end{itemize}

\paragraph{Limitations and considerations.}

\begin{itemize}
  \item \textbf{Notary trust.} Some ZK-TLS protocols (e.g., early TLSNotary) require a semi-trusted notary that participates in the session. More recent approaches use multi-party computation to distribute trust across multiple notaries, or leverage TEEs (trusted execution environments) to minimize trust assumptions.
  \item \textbf{Proof complexity.} Generating ZK proofs over TLS transcripts can be computationally expensive. Practical deployments may need to balance proof granularity against generation time and proof size.
  \item \textbf{Data freshness.} Unlike platform-issued credentials that can be updated server-side, ZK-TLS proofs reflect data at a specific point in time. The SEL agent must periodically regenerate proofs to maintain fresh SEV credentials.
  \item \textbf{Anti-scraping measures.} Platforms may deploy CAPTCHAs, rate limiting, or dynamic content that complicates automated ZK-TLS proof generation. However, since users are proving facts about their own accounts while logged in, many such measures are less restrictive than for anonymous scraping.
\end{itemize}

\paragraph{Hybrid issuance model.} In practice, SEL-DID deployments will likely use a hybrid model:

\begin{itemize}
  \item Where platforms actively participate as SEV issuers (e.g., through data portability APIs or explicit VC issuance services), users obtain platform-signed credentials as described in Section~\ref{sec:construction}.
  \item Where platforms do not cooperate, users fall back to ZK-TLS-based user-generated proofs.
  \item The SEV aggregation logic treats both credential types equivalently, weighting them by the strength of the underlying attestation (platform signature vs.\ ZK-TLS proof with specified notary trust model).
\end{itemize}

This hybrid approach ensures that SEL-DIDs remain functional even in a world where most platforms never become active credential issuers, while still benefiting from direct platform attestations when available.
...
\[
  \mathbf{y} = \bigparallel_{p \in \mathcal{P}'} \mathbf{y}_p,
\]
...
\subsection{Contextual Binding: Preventing Cross-Service Aggregation}
\label{sec:contextual_binding}

A critical privacy requirement for SEL-DIDs is that SEV proofs cannot be aggregated across different relying parties to construct a global ``social credit score.'' Without explicit countermeasures, colluding RPs could pool the proofs they receive, correlate users across services, and build comprehensive behavioral profiles that no individual RP was authorized to see. This ``Black Mirror'' scenario represents one of the most serious potential abuses of behavioral identity systems.

To prevent such aggregation, SEL-DIDs require \emph{contextual binding}: each SEV proof must be cryptographically tied to the specific relying party requesting it, such that the proof is meaningless (and unverifiable) outside that context.

\subsubsection{RP-Specific Proof Derivation}

Let $\mathsf{rp\_id}$ denote a unique, stable identifier for the relying party (e.g., a domain name, DID, or public key hash). When generating a SEV proof for RP $r$, the holder's agent computes an RP-specific salt:
\[
  \mathsf{salt}_r = H(\mathsf{did} \,\|\, \mathsf{rp\_id}_r \,\|\, \mathsf{epoch}),
\]
where $H$ is a cryptographic hash function and $\mathsf{epoch}$ is an optional time-bucketing parameter (e.g., current month) to limit temporal correlation.

The zero-knowledge proof $\pi_r$ is then constructed as:
\[
  \pi_r = \textsf{ZKProve}\big( \mathsf{salt}_r, \mathcal{E}, \mathcal{C}, \Pi_r \big),
\]
where the proof statement binds to both the RP-specific salt and the RP's policy $\Pi_r$. Critically, the proof includes a commitment to $\mathsf{salt}_r$ that can only be opened with knowledge of $\mathsf{rp\_id}_r$.

\subsubsection{Verification Binding}

To verify the proof, the RP must provide its own $\mathsf{rp\_id}_r$ as part of the verification:
\[
  \textsf{ZKVerify}\big( \pi_r, \mathsf{rp\_id}_r, \Pi_r \big) \stackrel{?}{=} 1.
\]

If a different RP $r'$ attempts to verify $\pi_r$ using $\mathsf{rp\_id}_{r'}$, the verification fails. This ensures that:
\begin{itemize}
  \item proofs are non-transferable between relying parties,
  \item the same user presenting proofs to $n$ different RPs produces $n$ unlinkable proof instances, and
  \item even if RPs collude and share all received proofs, they cannot verify each other's proofs or determine that two proofs originated from the same user.
\end{itemize}

\subsubsection{Unlinkability via Blinded Identifiers}

To further strengthen unlinkability, the proof may include a \emph{blinded pseudonym} derived from the RP salt:
\[
  \mathsf{nym}_r = H'(\mathsf{did} \,\|\, \mathsf{salt}_r),
\]
where $H'$ is a separate hash function (or the same function in a different domain). The RP receives $\mathsf{nym}_r$ as part of the authentication, which it can use for session management and rate limiting, but which is unlinkable to $\mathsf{nym}_{r'}$ presented to any other RP.

This construction ensures that the user's $\mathsf{did}$ itself need not appear in the proof at all---the RP only sees a context-specific pseudonym---further reducing correlation attack surface.

\subsubsection{Anti-Aggregation Properties}

The contextual binding mechanism provides the following guarantees against cross-service aggregation:

\begin{enumerate}
  \item \textbf{Proof isolation.} A proof $\pi_r$ generated for $\textsf{RP}_r$ is cryptographically meaningless to $\textsf{RP}_{r'}$; verification will fail.
  \item \textbf{Pseudonym unlinkability.} The pseudonyms $\mathsf{nym}_r$ and $\mathsf{nym}_{r'}$ are computationally indistinguishable from random, even to an adversary who knows both $\mathsf{rp\_id}_r$ and $\mathsf{rp\_id}_{r'}$, unless they also know the user's $\mathsf{did}$ and private key.
  \item \textbf{No global scoring.} Because each RP sees only its own contextually-bound view of the SEV (via predicate satisfaction, not raw values), and because proofs cannot be combined or correlated, no entity---including a coalition of all RPs---can reconstruct a user's full SEV or compute a ``global social score.''
  \item \textbf{Temporal compartmentalization.} The optional $\mathsf{epoch}$ parameter ensures that even within a single RP, proofs from different time periods are unlinkable unless the RP explicitly tracks session continuity.
\end{enumerate}

These properties directly address security goal G5 (Contextual binding) and provide a technical barrier against the ``Black Mirror'' scenario of pervasive behavioral scoring.
...
Let $\mathsf{did}$ be the holder's SEL-DID, and let $(\mathsf{sk}, \mathsf{pk})$ be the corresponding keypair.
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item computes the RP-specific salt $\mathsf{salt}_r = H(\mathsf{did} \,\|\, \mathsf{rp\_id}_r \,\|\, \mathsf{epoch})$ and blinded pseudonym $\mathsf{nym}_r$ as described in Section~\ref{sec:contextual_binding};
    \item constructs a contextually-bound ZK proof $\pi_r = \textsf{ZKProve}(\mathsf{salt}_r, \mathcal{E}, \mathcal{C}, \Pi_r)$ attesting that there exists $(\mathcal{E}, \mathcal{C})$ consistent with the committed SEV and credentials such that $\Pi(\mathcal{E}, \mathcal{C}) = 1$, bound to this specific RP.
  \end{enumerate}
  \item \textbf{DID Auth.} In parallel or sequentially, the agent computes a signature $\sigma = \textsf{Sign}_{\mathsf{sk}}(n \,\|\, \textsf{ctx})$, where $\textsf{ctx}$ binds the signature to this RP and session (e.g., includes the RP's domain and a timestamp).
  \item \textbf{Submission.} The holder sends to the RP:
  \[
    (\mathsf{nym}_r, \sigma, \pi_r, \textsf{aux}),
  \]
  where $\mathsf{nym}_r$ is the RP-specific blinded pseudonym (optionally, $\mathsf{did}$ may be included if the RP requires it for other purposes), and $\textsf{aux}$ may include non-sensitive metadata such as the chosen SEL policy identifier and any public parameters needed to verify $\pi_r$.
  \item \textbf{Verification.} The RP:
  \begin{enumerate}
    \item if $\mathsf{did}$ is provided, resolves it to obtain the associated DID document $\mathsf{DD}$ and public key $\mathsf{pk}$; otherwise, uses the blinded pseudonym $\mathsf{nym}_r$ as the session identifier;
    \item verifies the signature $\sigma$ against $\mathsf{pk}$ and challenge $n$;
    \item verifies the contextually-bound ZK proof $\pi_r$ using its own $\mathsf{rp\_id}_r$: $\textsf{ZKVerify}(\pi_r, \mathsf{rp\_id}_r, \Pi_r) \stackrel{?}{=} 1$.
  \end{enumerate}
  If all checks pass, the RP accepts the authentication and establishes a session associated with $\mathsf{nym}_r$ (or $\mathsf{did}$) and the policy level used. Importantly, the RP cannot share or replay $\pi_r$ to other services, as verification requires knowledge of the correct $\mathsf{rp\_id}$.
\end{enumerate}

This protocol provides passwordless authentication with two orthogonal assurances: key control and SEL policy satisfaction. The RP does not learn the specific platforms involved, the user's contacts, or detailed behavior, only that the SEV meets the specified criteria. Moreover, the contextual binding mechanism (Section~\ref{sec:contextual_binding}) ensures that proofs are non-transferable and that colluding RPs cannot aggregate SEV information across services to construct global behavioral profiles.
...
We aim to enable a user who has lost access to the private key $\mathsf{sk}$ associated with $\mathsf{did}$ to:

\begin{itemize}
  \item either bind a new keypair $(\mathsf{sk}', \mathsf{pk}')$ to the same DID (if the DID method supports key rotation under sufficient proof), or
...
\begin{enumerate}
  \item The user installs a new SEL agent and generates a fresh keypair $(\mathsf{sk}', \mathsf{pk}')$ and, if necessary, a new DID $\mathsf{did}'$.
...
\begin{itemize}
  \item \textbf{Coercive identity scoring.} States or corporations could attempt to use SEV-like constructions to build opaque credit- or loyalty-scoring systems that condition access to services on behavioral conformity.
  \item \textbf{Cross-service aggregation (``Black Mirror'' scenario).} Colluding relying parties could attempt to pool SEV proofs to construct comprehensive behavioral profiles and global ``social credit'' scores.
...
While no technical design alone can prevent all misuse, SEL-DIDs incorporate explicit countermeasures against the most serious abuses. In particular, the \emph{contextual binding} mechanism described in Section~\ref{sec:contextual_binding} provides a cryptographic barrier against cross-service aggregation: proofs are salted with the requesting RP's identifier and cannot be verified by other parties, pseudonyms are unlinkable across services, and even a coalition of all RPs cannot reconstruct a user's full SEV or correlate their activity. This makes global behavioral scoring technically infeasible without user cooperation.

Beyond cryptographic protections, governance and transparency measures can further reduce abuse risks. For example, open standards bodies can:
...
\subsection{Revocation and Privacy: The Nuclear Option}

A fundamental tension exists between the SEL-DID goal of building long-term behavioral continuity and the user's right to be forgotten. In some scenarios---duress, stalking, escape from abusive relationships, political persecution, or simply a desire to start fresh---a user may need to sever the link between their DID and their accumulated behavioral history entirely. We call this the ``Nuclear Option.''

\subsubsection{The Right to Sever}

Unlike traditional account deletion, where a central authority removes records, decentralized identity complicates forgetting: credentials may be cached by relying parties, zero-knowledge proofs may have been recorded, and the SEV itself may have left traces in on-chain or off-chain systems. SEL-DIDs must therefore provide a principled mechanism for users to abandon an identity without being traceably linked to a successor.

The Nuclear Option proceeds as follows:

\begin{enumerate}
  \item \textbf{Key rotation to a fresh DID.} The user generates an entirely new keypair $(sk', pk')$ and a new DID $\mathsf{did}'$ with no cryptographic or on-chain linkage to the original $\mathsf{did}$.
  \item \textbf{Credential non-transfer.} Critically, the user does \emph{not} migrate or re-issue SEV credentials from $\mathcal{C}$ to the new DID. The old credentials remain bound to the old $\mathsf{did}$ and are effectively orphaned.
  \item \textbf{Revocation of old credentials.} Where supported, the user issues revocation statements for all credentials in $\mathcal{C}$, signaling to relying parties that the old SEL-DID should no longer be trusted. This is optional but recommended to prevent the old identity from being hijacked.
  \item \textbf{Platform-level disconnection.} Where possible, the user unlinks or deletes platform accounts that contributed to the old SEV. This is outside the SEL-DID protocol proper but is part of a comprehensive right-to-be-forgotten exercise.
  \item \textbf{Cold start on the new identity.} The user begins accumulating SEV evidence from scratch on $\mathsf{did}'$, accepting the cold-start penalty as the cost of privacy.
\end{enumerate}

\subsubsection{Design Constraints for Unlinkability}

For the Nuclear Option to be meaningful, the system must ensure that:

\begin{itemize}
  \item \textbf{No mandatory linkage.} Recovery and migration flows must be opt-in. A user who chooses \emph{not} to prove continuity between $\mathsf{did}$ and $\mathsf{did}'$ should face no automatic penalties beyond the cold-start cost.
  \item \textbf{No global identity registry.} SEL-DIDs must not depend on a central database that tracks all DIDs and could be queried to find successors. Each DID is independently generated and self-certifying.
  \item \textbf{Minimal metadata leakage.} Credential schemas and ZK proofs should be designed so that repeated use of the same platforms on a new DID does not create trivially linkable fingerprints. For example, if a user's old and new SEVs both include credentials from the same three platforms with similar account ages, this should not be directly observable by relying parties.
  \item \textbf{Time-bounded caching.} Relying parties should be encouraged (or required by policy) to expire cached SEV proofs and not retain indefinite records of which DIDs satisfied which policies.
\end{itemize}

\subsubsection{Trade-offs and Limitations}

The Nuclear Option is not free. A user who exercises it:

\begin{itemize}
  \item loses all accumulated SEV-based reputation and must rebuild from zero;
  \item may be unable to access services that require high-entropy SEVs until sufficient time has passed;
  \item cannot prove continuity to relying parties who might otherwise offer preferential treatment to long-standing identities.
\end{itemize}

These costs are intentional: they are the same costs that make Sybil attacks expensive. The difference is that the legitimate user is making a deliberate, informed choice to pay this cost in exchange for privacy, rather than being forced into it by an adversary.

In extreme cases---such as fleeing domestic abuse or political persecution---the cold-start penalty may itself be a barrier to safety. Future work should explore mechanisms for privacy-preserving ``fresh start'' attestations that can be issued by trusted third parties (e.g., shelters, human rights organizations) to bootstrap a new SEL-DID without revealing linkage to the old one, while still providing some Sybil resistance.

\subsubsection{Relation to GDPR and Data Protection}

The Nuclear Option aligns with the ``right to erasure'' (Article 17) under GDPR and similar data protection regimes. However, SEL-DIDs add complexity because:

\begin{itemize}
  \item credentials are held in user-controlled vaults, so ``deletion'' is largely under user control;
  \item relying parties may have cached proofs or policy decisions that persist beyond credential revocation;
  \item on-chain records (if any) may be immutable by design.
\end{itemize}

Deployments should clearly document which data is stored where, under whose control, and what deletion or revocation mechanisms are available. Where on-chain records are used, designs should prefer commitments and hashes over plaintext, and should support revocation lists or expiration semantics that allow old records to become meaningless over time.
...
\begin{enumerate}
  ...
  \item \textbf{Financial stake.} Each vote requires a small payment (e.g., \$1 equivalent), processed through a regulated payment provider that implicitly confirms possession of a local bank account. This introduces economic friction without requiring explicit KYC documents.
...
\begin{itemize}
  \item \textbf{Sybil resistance without biometrics.} An attacker wishing to cast $n$ fake votes must maintain $n$ aged social accounts, control $n$ local bank accounts, physically travel to the municipality $n$ times, and pay the per-vote fee $n$ times. For modest $n$, this is already expensive; for large $n$, it is prohibitive.
...
\subsection{Assurance Tiers for Relying Parties}
\label{sec:assurance_tiers}

To simplify adoption, we propose a standardized set of \emph{Assurance Tiers} that map SEV thresholds to graduated trust levels. Rather than requiring each Relying Party (RP) to define complex, bespoke policies from scratch, these tiers provide ready-made policy templates that can be referenced by name or further customized.

\begin{table}[h]
  \centering
  \begin{tabular}{p{1.8cm} p{2.2cm} p{4.5cm} p{5.5cm}}
    \hline
    Tier & Trust Level & SEV Requirements & Typical Use Cases \\
    \hline
    Tier 1 &
    Low Trust &
    \begin{tabular}[t]{@{}l@{}}
      $\bullet$ $C(t) \geq 0.2$ \\
      $\bullet$ $\geq 1$ platform credential \\
      $\bullet$ Any account age
    \end{tabular} &
    Anonymous comments, public forums, newsletter signups, read-only access to resources \\
    \hline
    Tier 2 &
    Medium Trust &
    \begin{tabular}[t]{@{}l@{}}
      $\bullet$ $C(t) \geq 0.4$ \\
      $\bullet$ $\geq 2$ independent platforms \\
      $\bullet$ $\geq 1$ account aged $>6$ months \\
      $\bullet$ Basic behavioral consistency
    \end{tabular} &
    Social platform participation, content creation, basic e-commerce, community membership \\
    \hline
    Tier 3 &
    High Trust &
    \begin{tabular}[t]{@{}l@{}}
      $\bullet$ $C(t) \geq 0.65$ \\
      $\bullet$ $\geq 3$ independent platforms \\
      $\bullet$ $\geq 2$ accounts aged $>1$ year \\
      $\bullet$ Demonstrated social graph \\
      $\bullet$ Consistent behavioral patterns
    \end{tabular} &
    DAO governance voting, quadratic funding, financial services onboarding, moderation roles \\
    \hline
    Tier 4 &
    Human-Only / Critical &
    \begin{tabular}[t]{@{}l@{}}
      $\bullet$ $C(t) \geq 0.85$ \\
      $\bullet$ $\geq 4$ independent platforms \\
      $\bullet$ $\geq 2$ accounts aged $>3$ years \\
      $\bullet$ Dense, reciprocal social graph \\
      $\bullet$ Long-term behavioral stability \\
      $\bullet$ Cross-platform consistency check
    \end{tabular} &
    High-stakes governance (constitutional votes), large financial transactions, civic elections, critical infrastructure access \\
    \hline
  \end{tabular}
  \caption{Standardized SEL-DID Assurance Tiers mapping SEV thresholds to trust levels. RPs can adopt tiers by reference or customize thresholds for domain-specific needs.}
  \label{tab:assurance_tiers}
\end{table}

Each tier is defined by a combination of:
\begin{itemize}
  \item \textbf{Continuity score threshold} $C(t)$: the minimum value of the identity continuity function at verification time.
  \item \textbf{Platform diversity}: the number of independent platforms contributing valid SEV credentials.
  \item \textbf{Account longevity}: minimum age requirements for underlying platform accounts.
  \item \textbf{Behavioral indicators}: qualitative requirements on social graph structure and behavioral consistency.
\end{itemize}

\subsubsection{Tier Selection Guidelines}

RPs should select tiers based on the consequences of identity fraud in their context:

\begin{description}
  \item[Tier 1] is appropriate when the cost of a Sybil attack is low and the primary goal is to filter out trivially automated accounts. False negatives (admitting some fake identities) are acceptable.
  \item[Tier 2] suits applications where user-generated content or limited resources are at stake, but the system can tolerate occasional bad actors and rely on additional moderation.
  \item[Tier 3] is designed for governance and resource allocation systems where each identity's vote or claim has material impact. False positives (rejecting legitimate users) should be minimized but are less harmful than Sybil infiltration.
  \item[Tier 4] is reserved for critical decisions where even a single forged identity could cause significant harm. This tier may exclude new users or those with limited digital presence; such exclusion should be mitigated by offering alternative verification paths (e.g., in-person ceremonies, guardian attestations).
\end{description}

\subsubsection{Interoperability and Customization}

Assurance Tiers are intended as a shared vocabulary, not a rigid mandate. RPs may:
\begin{itemize}
  \item reference a tier by identifier (e.g., ``require SEL-DID Tier 3'') in authentication policies;
  \item adjust individual thresholds (e.g., raise $C(t)$ for Tier 2 to 0.5 in a high-risk domain);
  \item combine tier requirements with additional factors (e.g., Tier 3 plus a financial stake or guardian signature);
  \item define application-specific ``Tier 2.5'' policies that interpolate between standard tiers.
\end{itemize}

Standardization bodies may publish versioned tier definitions as part of the broader SEL-DID specification, enabling credential wallets and verification libraries to implement tier checks out of the box.
